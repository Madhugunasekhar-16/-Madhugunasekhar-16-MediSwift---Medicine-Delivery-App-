      ...
      <div className="flex items-center gap-3 md:hidden z-[101]">
        {user && user.role !== "admin" && (
          <Link to="/cart" className="relative text-blue-600 text-xl p-2">
            <FaShoppingCart />
            {cartCount > 0 && (
              <span className="absolute top-0 right-0 bg-red-500 text-white text-[9px] w-4 h-4 flex items-center justify-center rounded-full">{cartCount}</span>
            )}
          </Link>
        )}
        ...
        {user ? (
          <div className="flex items-center ml-2">
            {user.role === "admin" ? (
              ...
              <>
                <Link to="/cart" className={`relative ${navLinkClasses}`}><FaShoppingCart /> Cart {cartCount > 0 && <span className="ml-1 bg-red-500 text-white text-[10px] px-1.5 rounded-full">{cartCount}</span>}</Link>
            ...
            <button onClick={handleLogout} className="ml-3 bg-red-500 text-white px-4 py-2 rounded-lg text-[0.85rem] font-semibold hover:bg-red-600">Logout</button>
          </div>
        ) : (
          <div className="flex items-center gap-3 ml-4">
            <Link to="/login" className="no-underline border-blue-600 text-blue-600 px-5 py-1.5 rounded-lg font-semibold hover:bg-blue-50">Login</Link>
            <Link to="/register" className="no-underline bg-blue-600 text-white px-5 py-1.5 rounded-lg font-semibold hover:bg-blue-700">Register</Link>
          </div>
        )}
      </div>
